...
resource "infisical_secret_folder" "dns_credential_gateway" {
  ...
  description      = "Signing key generated by cloudlab for authenticated DoH and DoT credentials."
}
...
resource "infisical_secret" "dns_credential_key" {
  ...
  name         = "CREDENTIAL_KEY"
  value        = data.terraform_remote_state.cloudlab.outputs.dns_credential_key
}
